...
def test_dataset_operations(client_as_admin, db_session):
    ...
    response = client_as_admin.delete(f"/api/v1/datasets/{new_dataset_id}")
    assert response.status_code == 200
    ...
    response = client_as_admin.get(f"/api/v1/datasets/{new_dataset_id}/summary")
    assert response.status_code == 200
...
def test_new_dataset_features(client_as_admin, db_session, monkeypatch):
    ...
    resp_details = client_as_admin.get(f"/api/v1/datasets/{dataset_id}")
    ...
    resp_preview = client_as_admin.get(f"/api/v1/datasets/{dataset_id}/preview")
    ...
    resp_stats = client_as_admin.get(f"/api/v1/datasets/{dataset_id}/statistics")
...
def test_dataset_active_management(client_as_admin, db_session):
    ...
    response = client_as_admin.get("/api/v1/datasets/config")
    assert response.status_code == 200
    assert response.json()["max_active_datasets"] == "1"

    # 2. Test PUT configuration
    resp_put = client_as_admin.put("/api/v1/datasets/config", json={"max_active_datasets": "2"})
    ...
    # 3. Activate ds1
    resp_act1 = client_as_admin.post("/api/v1/datasets/activate", json={"dataset_id": ds1.id})
    assert resp_act1.status_code == 200
    assert resp_act1.json()["is_active"] is True

    # 4. Activate ds2
    resp_act2 = client_as_admin.post("/api/v1/datasets/activate", json={"dataset_id": ds2.id})
    ...
    resp_act3 = client_as_admin.post("/api/v1/datasets/activate", json={"dataset_id": ds3.id})
    ...
    resp_act = client_as_admin.get("/api/v1/datasets/active")
    ...
    resp_deact = client_as_admin.post("/api/v1/datasets/deactivate", json={"dataset_id": ds2.id})
...
def test_dataset_permanent_delete(client_as_admin, db_session):
    ...
    client_as_admin.get("/api/v1/datasets/")
    ...
    response = client_as_admin.delete(f"/api/v1/datasets/{ds_id}/permanent")
    assert response.status_code == 200
    ...
    response = client_as_admin.delete(f"/api/v1/datasets/{ds1_id}")
    assert response.status_code == 200
    ...
    client_as_admin.delete(f"/api/v1/datasets/{ds1_id}/permanent")
    client_as_admin.delete(f"/api/v1/datasets/{ds2_id}/permanent")
